        ...
        public virtual void OnFrame(Dictionary<string, AnimationMetaData> activeAnimationsByAnimCode, float dt)
        {
            ...
            for (int i = 0; i < anims.Length; i++)
            {
                ...
                if (wasActive && !anim.Active)
                {
                    ...
                    else if (anim.Animation.OnActivityStopped == EnumEntityActivityStoppedHandling.Stop)
                    {
                        anim.Stop();
                        //For the frame slider to work, it's important that stopped animations do not actually get removed.
                        //activeAnimationsByAnimCode.Remove(anim.Animation.Code);
                    }

                    else if (anim.Animation.OnActivityStopped == EnumEntityActivityStoppedHandling.PlayTillEnd)
                    {
                ...
                {
                    //We don't do any 'easing' in the editor - So just stop the animation.
                    anim.Stop();
                    //anim.EaseOut(dt);
                }
