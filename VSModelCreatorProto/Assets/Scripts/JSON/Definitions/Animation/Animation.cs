...
namespace VSMC
{
    ...
    public class Animation
    {
        ...
        */
        ...
        public Animation(string codeAndName)
        {
            QuantityFrames = 30;
            Code = codeAndName;
            Name = codeAndName;
            KeyFrames = new AnimationKeyFrame[0];
        }
        ...
        {
            KeyFrames = KeyFrames.OrderBy(x => x.Frame).ToArray();
        }
        ...
        public void GenerateAllFrames(ShapeElement[] rootElements, Dictionary<int, AnimationJoint> jointsById, bool recursive = true)
        {
            ...
            {
                Debug.LogWarning("Attempting animation with no keyframes.");
            ...
            {
                jointsDone.Clear();
                GenerateFrame(i, resolvedKeyFrames, rootElements, jointsById, Matrix4x4.identity, resolvedKeyFrames[i].RootElementTransforms, recursive);
            }

            /* Unused AnimationFrame code.
            for (int i = 0; i < resolvedKeyFrames.Length; i++)
            {
        ...
        protected void GenerateFrame(int indexNumber, AnimationFrame[] resKeyFrames, ShapeElement[] elements, Dictionary<int, AnimationJoint> jointsById, Matrix4x4 modelMatrix, List<ElementPose> transforms, bool recursive = true)
        {
            ...
            for (int i = 0; i < elements.Length; i++)
            {
                ...
                Matrix4x4 animModelMatrix = Matrix4x4.identity * modelMatrix;
                animModelMatrix = animModelMatrix * element.GetLocalTransformMatrix(Version, null, animTransform);
        ...
        /// <summary>
        /// Will return a keyframe if it exists. If not, will create it and add it to the animation.
        /// </summary>
        public AnimationKeyFrame GetOrCreateKeyFrame(int frame)
        {
            AnimationKeyFrame kf = KeyFrames.FirstOrDefault(x => x.Frame == frame);
            ...
            KeyFrames = KeyFrames.Append(kf);
            return kf;
        }

        /// <summary>
        /// Removes any empty keyframes. This will also remove any empty keyframe elements.
        /// </summary>
        public void RemoveAnyEmptyKeyFrames()
        {
            List<AnimationKeyFrame> keeps = new List<AnimationKeyFrame>();
            foreach (AnimationKeyFrame kf in KeyFrames)
            {
                kf.RemoveAnyEmptyKeyFrameElements();
                if (kf.Elements.Count != 0) keeps.Add(kf);
            }
            KeyFrames = keeps.ToArray();
        }
    }
}